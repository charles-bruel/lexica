...
impl super::data::Program {
    pub fn apply(&self, input: Vec<Letter>) -> Vec<Letter> {
    ...
    pub fn apply_vec(&self, input: Vec<Vec<Letter>>) -> Vec<Vec<Letter>> {
...
impl super::data::RuleByte {
    pub fn apply(&self, input: Vec<Letter>) -> Vec<Letter> {
        match self.transformations.len() == 1 {
            true => self.apply_single(input),
            false => self.apply_multi(input),
        }
    }

    fn apply_single(&self, input: Vec<Letter>) -> Vec<Letter> {
        if self.transformations[0].predicate.len() == 0 {
            return self.apply_empty_predicate(input);
        } else {
            return self.apply_single_simple(input);
        }
    }

    fn apply_empty_predicate(&self, input: Vec<Letter>) -> Vec<Letter> {
        let mut result = input.clone();
        let mut i: usize = 0;
        while i < result.len() {
            ...
            if flag {
                ...
                let temp = rule.transform(&Letter { value: 0 });//Dummy input; there are better ways to do this
                result.insert(i + 1, temp.unwrap());
                i += 1;
            }
    ...
    fn apply_single_simple(&self, input: Vec<Letter>) -> Vec<Letter> {
        let mut result = input.clone();
        let mut i: usize = 0;
        while i < result.len() {
            let mut flag = false;
    ...
    fn apply_multi(&self, input: Vec<Letter>) -> Vec<Letter> {
...
pub fn from_string(program: &Program, input: String) -> Vec<Letter> {
    ...
    while string.len() > 0 {
        ...
        depth += 1;
        if depth > 1024 {
            panic!("Could not convert string \"{0}\", got to \"{1}\"", input, string);
        }
    }
    return result;
}