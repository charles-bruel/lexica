...
fn test_positive_negative_predicate() {
    ...
    let pos_test = super::data::Letter { value: key };
    let neg_test_1 = super::data::Letter {
        value: (key & !neg_mask) | !(key & neg_mask),
    };
    let neg_test_2 = loop {
        let neg = random_letter();
        if neg.value & mask != key {
            break neg;
        }
    };

    let predicate =
        create_positive_negative_predicate(mask, key & mask, vec![neg_mask], vec![neg_key]);
...
fn test_diacritics_d() {
    ...
    assert!(is_anagram(
        String::from("1aAᵃ"),
        letter.get_symbol(&program).unwrap()
    ));
}
...
fn test_unknown_command_error_a() {
    ...
    match result {
        Ok(_) => panic!(),
        Err(v) => assert!(
            v.error_type == ConstructorErrorType::UnknownCommandError
                && v.line_number_user_program == LineNumberInformation::Raw(1)
        ),
    }
}
...
fn test_unknown_command_error_b() {
    ...
    match result {
        ...
        Err(v) => assert!(
            v.error_type == ConstructorErrorType::UnknownCommandError
                && v.line_number_user_program == LineNumberInformation::Raw(2)
        ),
    }
}
...
fn test_hanging_section_error_a() {
    ...
    match result {
        Ok(_) => panic!(),
        Err(v) => assert!(
            v.error_type == ConstructorErrorType::HangingSection
                && v.line_number_user_program == LineNumberInformation::Raw(1)
        ),
    }
}
...
fn test_malformed_feature_def_error_a() {
    ...
    match result {
        Ok(_) => panic!(),
        Err(v) => assert!(
            v.error_type == ConstructorErrorType::MalformedDefinition
                && v.line_number_user_program == LineNumberInformation::Raw(2)
        ),
    }
}
...
fn test_missing_node_error() {
    ...
    match result {
        Ok(_) => panic!(),
        Err(v) => assert!(
            v.error_type == ConstructorErrorType::MissingNode
                && v.line_number_user_program == LineNumberInformation::Raw(2)
        ),
    }
}
...
fn test_syllable_conversion_a() {
    let prog = construct(
        &load_from_file(&String::from("test-data/full-ipa.lsc"), false)
...
fn simple_test_helper(rule: &str, input: &str) -> String {
    ...
    to_string(
        &prog,
        prog.apply(from_string(&prog, &String::from(input)).unwrap())
            .unwrap(),
    )
    .unwrap()
}
...
fn create_diacritic_test_program() -> Program {
    construct(
        &load_from_file(&String::from("test-data/diacritics-test.lsc"), false)
            .expect("Error reading file"),
    )
    .unwrap()
}
...
fn create_int_test_1() -> Program {
    let defs =
        load_from_file(&String::from("test-data/full-ipa.lsc"), false).expect("Error reading file");
    let rules = load_from_file(&String::from("test-data/int-test-1.lsc"), false)
        .expect("Error reading file");
    construct(&format!("{0}\n{1}", defs, rules)).unwrap()
}
...
fn create_int_test_2() -> Program {
    let defs =
        load_from_file(&String::from("test-data/full-ipa.lsc"), false).expect("Error reading file");
    let rules = load_from_file(&String::from("test-data/int-test-2.lsc"), false)
        .expect("Error reading file");
    construct(&format!("{0}\n{1}", defs, rules)).unwrap()
}
